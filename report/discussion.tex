...
configuration of error tags. The idea was to give the synthesis tool
as much freedom as possible in doing the mapping of the expression
onto the 4-input LUTs on the Xilinx FPGA. Using this solution, we got
got 17 LUTs for the system without output selection and ECC, with an
...
easy to interpret or . An advantage of this solution was that it was
very easy to read, so it might 

 gave the syntesis tool 


 and then let the
synthesis tool find a good mapping. This approach also used 19 LUTs, was not that flexible,
